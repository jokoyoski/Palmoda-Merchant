...
const completionPercent = Math.round((stepsCompleted / totalSteps) * 100);
  ...
  <h1 className="text-black font-semibold text-sm">Complete Your Brand Profile</h1>
  ...
  <div className="w-full mt-4">
    ...
    <p className="text-right text-xs text-gray-600 mt-1">{completionPercent}%</p>
  </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-5 mb-4">
    ...
    <div className="border border-gray-200 rounded-[5x] py-4 px-6 flex items-center gap-3">
      <div>
        {basicInfoComplete ? (
          <span className="text-green-600 bg-green-100 p-2 rounded-[50%] text-lg">✔</span>
        ) : (
          <span className="text-yellow-500 bg-yellow-100 p-2 rouned-[50%] text-lg">●</span>
        )}
      </div>

      <div>
        <p className="text-sm font-semibold text-black">Basic Information</p>
  ...
  <Link href="/brand-profile" className='text-xs mt-4 px-4 py-2 mb-5 border border-black text-black uppercase bg-inherit'>
      Go to Brand Profile
    </Link>
</div>
